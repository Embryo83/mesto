...
const popupEditProfile = new PopupWithForm('.popup_type_edit', editSubmitHandler);
const popupAddCard = new PopupWithForm('.popup_type_add', addCardSubmitHandler);
const popupConfirmDelete = new PopupConfirm('.popup_type_delete', confirmSubmitHandler);
const popupChangeAvatar = new PopupWithForm('.popup_type_avatar', avatarSubmitHandler);
...
Promise.all(getData)
  .then((data) => {
    ...
    cardsGrid = new Section({
      items: cardsData,
      renderer: (item) => {
        const card = new Card(item, '#elements-template', userData._id, openCardImage, openPopupConfirm, likeHandler);
